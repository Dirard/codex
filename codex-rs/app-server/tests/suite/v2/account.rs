...
fn create_config_toml(codex_home: &Path, params: CreateConfigTomlParams) -> std::io::Result<()> {
    ...
    let use_builtin_openai_provider = matches!(params.requires_openai_auth, Some(true))
        && params.model_provider_id.is_none()
        && params.extra_provider_config.is_none();
    let model_provider_id = params.model_provider_id.unwrap_or_else(|| {
        if use_builtin_openai_provider {
            "openai".to_string()
        } else {
...
fn get_account_params(refresh_token: bool) -> GetAccountParams {
    GetAccountParams {
        refresh_token,
        model_provider: None,
    }
...
fn get_account_params_for_provider(
...
async fn logout_account_removes_auth_and_notifies() -> Result<()> {
    ...
    let get_id = mcp
        .send_get_account_request(get_account_params(false))
        .await?;
...
async fn account_read_refresh_token_is_noop_in_external_mode() -> Result<()> {
    ...
    let get_id = mcp
        .send_get_account_request(get_account_params(true))
        .await?;
    let get_resp: JSONRPCResponse = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_response_message(RequestId::Integer(get_id)),
...
#[tokio::test]
async fn account_read_with_model_provider_uses_latest_config_and_skips_custom_refresh() -> Result<()>
{
    let codex_home = TempDir::new()?;
    create_config_toml(codex_home.path(), CreateConfigTomlParams::default())?;
    write_chatgpt_auth(
...
async fn account_read_unknown_model_provider_returns_invalid_request() -> Result<()> {
    ...
    assert_eq!(err.error.message, "unknown modelProvider: missing_provider");
    Ok(())
}
...
#[tokio::test]
async fn account_read_with_openai_model_provider_preserves_auth_behavior() -> Result<()> {
    let codex_home = TempDir::new()?;
    create_config_toml(
        codex_home.path(),
        CreateConfigTomlParams {
...
async fn get_account_no_auth() -> Result<()> {
    ...
    let params = get_account_params(false);
