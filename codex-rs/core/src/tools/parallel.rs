        ...
        async move {
            ...
            let mut outcome = match result {
                Ok(result) => {
                    ...
                    let made_progress = result.result.success_for_logging();
                    ToolCallResult {
                        response: result.into_response(),
                        made_progress,
                    }
                }
                Err(FunctionCallError::Fatal(message)) => return Err(CodexErr::Fatal(message)),
                Err(other) => ToolCallResult {
            ...
            recorder.attach_direct_call_to_output(&mut outcome.response.item, recorded_call);
            Ok(outcome)
        }
    }
