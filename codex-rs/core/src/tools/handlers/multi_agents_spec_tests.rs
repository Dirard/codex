...
fn wait_agent_tool_v2_uses_timeout_only_summary_output() {
    ...
    assert_eq!(
        properties
            ...
            .and_then(|schema| schema.description.as_deref()),
        Some(
            "Absolute deadline in milliseconds. Omit for the 1800000 ms default; pass 0 for one immediate check. Positive values must be between 10000 and 3600000. Prefer longer waits (minutes) to avoid busy polling."
        )
    );
