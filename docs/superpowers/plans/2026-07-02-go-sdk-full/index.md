...
| 5G | `stage-05g-package-source-hermeticity.md` | stages 0-5 | release-owned no-network helper sources for runtime package layout | `rg` and zsh package inputs are materialized as verified helper-root payloads without DotSlash/package-cache/network during Go SDK CI and Stage 7 staging, or Stage 6 runtime staging is explicitly blocked |
