...
for target in expected_targets:
    ...
    missing_smokes = sorted(required_smokes - smoke_tests)
    if missing_smokes:
        raise SystemExit(f"{target} packageArchive smoke suite missing {missing_smokes}")
...
- [ ] Verify Stage 5G produced a release-owned no-network helper source for managed `rg`, zsh, and archive `zstd`, or that archive staging has a clear fail-fast preinstalled-`zstd` prerequisite before package assembly. If Stage 5G marked runtime staging blocked, Stage 7 must stop before claiming real-runtime release readiness.
