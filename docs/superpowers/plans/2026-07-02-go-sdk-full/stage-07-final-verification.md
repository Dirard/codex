...
if metadata.get("notReleaseReady"):
    raise SystemExit(
        "sdk.yml aggregate evidence is explicitly not release-ready: "
        f"{metadata.get('notReleaseReadyTargets') or []}"
    )
...
if fixture_mode:
    ...
    raise SystemExit(
        "non-publishing fixture evidence is audit/preflight evidence only; "
        "final release readiness requires real non-fixture releaseArtifactEvidence"
    )
else:
