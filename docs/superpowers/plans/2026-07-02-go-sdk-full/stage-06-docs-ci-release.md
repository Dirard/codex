...
- [ ] Add real stdio app-server tests with these exact function names:
  ...
  - `TestRealAppServerUnauthenticatedAccountRead`: release runtime returns `account: null` with the manifest-backed `requiresOpenaiAuth` value without live auth/backend traffic or localhost auth override.
