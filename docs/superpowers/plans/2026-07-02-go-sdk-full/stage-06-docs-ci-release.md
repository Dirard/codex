...
Stage 6 depends on `stage-05g-package-source-hermeticity.md`. Do not start Stage 6 runtime packaging, staging scripts, CI, or Stage 7 final verification unless Stage 5G already proved no-network, release-owned helper payload sources for managed `rg` and zsh. If Stage 5G blocked runtime staging, keep Stage 6 docs/examples work separate and do not claim real-runtime release readiness.
...
2. Runtime package-layout source and collector: Bazel target, package-builder parity contract, helper payload materialization, no-network staging tests, and no workflow wiring.
...
- [ ] Before wiring workflow jobs, confirm Stage 5G is complete and add the Bazel runtime layout collector target:
  - create `//codex-rs/cli:codex_go_sdk_runtime_layout` in `codex-rs/cli/BUILD.bazel`
  - the target must materialize the complete install-context package layout expected by `InstallContext::from_exe`: `codex-package.json`, `bin/codex[.exe]`, concrete helper payloads under `codex-resources/`, and concrete managed PATH payloads under `codex-path/`
  - the target must be tied to the repository's existing package-builder source of truth. Either derive the collector layout from the same package-builder layout contract used by `.github/scripts/build-codex-package-archive.sh` and `scripts/codex_package/layout.py`, or add a required parity test that builds an actual current release-path `codex-package-*.tar.gz`, expands it, and diffs metadata, helper placement, executable names, and resource paths against the collector output for the same target. CI must not certify a Bazel-only synthetic layout that can diverge from the package shipped by the release workflows.
  - it must include the same-checkout entrypoint, Linux `bwrap`, Windows helpers, `codex-package.json`, `codex-path/<rg name>`, non-Windows `codex-resources/zsh/bin/zsh`, and any V8/package inputs from Bazel build outputs, runfiles, or checked-in files. Managed `rg` and zsh must come from the release-owned helper source established by Stage 5G, not from DotSlash/package-cache during staging.
  ...
  - it must not use DotSlash/package-builder/prebuilt downloads during Go SDK CI or Stage 7. If the Stage 5G no-network helper source is missing, stale, or incomplete, Stage 6 runtime packaging is blocked; do not add a fallback fetcher, ambient cache dependency, or placeholder helper directory.
  ...
  - verify locally on Linux/macOS with `bazel build //codex-rs/cli:codex_go_sdk_runtime_layout`; on native Windows, run the Stage 6 PowerShell bootstrap/export contract before any local Bazel command. CI/staging scripts must use the repo wrapper form `./.github/scripts/run-bazel-ci.sh --remote-download-toplevel -- build <args> -- //codex-rs/cli:codex_go_sdk_runtime_layout`.
- [ ] Modify `.github/workflows/sdk.yml`:
  ...
  - the staging helper must copy or materialize every resource that the runtime resolves from the package root recognized by `InstallContext::from_exe`, including `codex-resources/*` such as bundled `zsh` and Linux `bwrap`, `codex-path/*` such as bundled `rg`, and `codex-package.json`; if the current Bazel output only exposes a bare binary or Stage 5G did not create materialized helper inputs, the stage must fail until the exact `//codex-rs/cli:codex_go_sdk_runtime_layout` collector target produces the complete layout
  - before wiring CI, add the single supported local-only package resource source: a Bazel/runfiles install-layout collector target named `//codex-rs/cli:codex_go_sdk_runtime_layout`. The target must materialize the entrypoint, managed `rg`, zsh, Linux `bwrap`, Windows helpers, `codex-package.json`, and any V8/package inputs from same-checkout build outputs, Bazel runfiles, or checked-in files, while proving parity with the existing package-builder layout and consuming the no-network helper inputs from Stage 5G. The Go SDK staging scripts must consume this target and must not expose a second local-prebuilt/package-builder route for CI or Stage 7. Any future alternative staging architecture must be a separate reviewed stage before it can replace this route.
  - runtime staging for CI and Stage 7 must be network-hermetic: it may use only the `//codex-rs/cli:codex_go_sdk_runtime_layout` tree output, same-checkout build outputs, Bazel-materialized runfiles, checked-in files, or a fail-fast preinstalled `zstd` prerequisite for archive writing. It must not fetch DotSlash, V8, `rg`, zsh, `zstd`, helper archives, or package resources during Go SDK CI or final verification. The stage must add a no-network script test with downloads disabled and an empty DotSlash/package cache proving staging succeeds from the Bazel layout collector and fails if any required layout input is missing.
...
- [ ] Create `.github/scripts/stage-codex-runtime.sh` before wiring the Go SDK workflow. Contract:
  ...
  - stages an install-context-compatible package layout under `--out`, with package root `--out`, metadata file `--out/codex-package.json`, executable at `--out/bin/codex` or `--out/bin/codex.exe`, resources under `--out/codex-resources/`, and managed PATH helpers under `--out/codex-path/`; `CODEX_EXEC_PATH` must point to `--out/bin/codex[.exe]`. Default staging must be copied from the Bazel tree output of `//codex-rs/cli:codex_go_sdk_runtime_layout`, not an ad hoc flat directory and not a package-builder/prebuilt fallback. The `--release-package-archive` mode must build or consume the current release package-builder archive for the same Linux/macOS/Windows target, unpack it into the same install-context layout, require `--cargo-profile release` when it builds the archive, reject `dev`, and emit metadata that identifies the source as `packageArchive` and `cargoProfile` as `release`.
  - never downloads runtime/package resources during CI or Stage 7 verification. The helper must fail if the Bazel layout collector is unavailable, incomplete, not parity-checked against the package-builder layout, or resolves a required input through DotSlash/network/package-cache lookup. Add script tests with network disabled and an empty DotSlash/package cache so accidental live fetches fail before CI; these tests must also prove concrete managed `rg` and non-Windows `zsh` payloads are present from explicit release-owned Bazel/runfiles inputs or that staging is blocked by a prerequisite packaging-source gap before release-shaped staging is accepted.
