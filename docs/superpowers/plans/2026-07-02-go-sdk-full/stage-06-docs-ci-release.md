...
- [ ] Before wiring workflow jobs, confirm Stage 5G is complete and add the Bazel runtime layout collector target:
  ...
  - it must not use DotSlash/package-builder/prebuilt downloads during Go SDK CI or Stage 7. If the Stage 5G no-network helper source or verified helper-root manifest is missing, stale, or incomplete, Stage 6 runtime packaging is blocked; do not add a fallback fetcher, ambient cache dependency, or placeholder helper directory.
